...
FROM Windows IMPORT
    ...
    (* proc *)  OpenWindow, SetCursor, WriteChar, WriteString, Hide,
                ...
                RequestPageChangeNotification, PutOnPage, GetScreenSize;
...
VAR
    ...
    ScreenRows, ScreenColumns: CARDINAL;
...
PROCEDURE MoveWindow (w: Window;  yrel: RowRange;  xrel: ColumnRange);
    ...
    BEGIN
        R := WindowLocation(w);
        WITH R DO
            SetMouseCursorLimits (yrel, ScreenRows - 1 - bottom + yrel + top,
                                xrel, ScreenColumns - 1 - right + xrel + left);
        END (*WITH*);
...
PROCEDURE ContinueMove (x: ColumnRange;  y: RowRange);
    ...
    BEGIN
        ...
        ELSE
            ...
            SetMouseCursorLimits (0, ScreenRows-1, 0, ScreenColumns-1);
...
PROCEDURE DisplayWindowList (dummy: Window;  Y: RowRange;  X: ColumnRange);
    ...
    BEGIN
        ...
        IF Y > ScreenRows - count - 2 THEN
            Y := ScreenRows - count - 2;
        END (*IF*);
        IF X < 2 THEN
            X := 2;
        ELSIF X > ScreenColumns - 20 THEN
            X := ScreenColumns - 20;
        END (*IF*);
        HideMouseCursor;
...
PROCEDURE AddCapabilities (UIW: UIWindow;  CS: CapabilitySet);
    ...
    BEGIN
        ...
        IF (wescape IN CS) AND (maxcol >= 3) THEN
            SetCursor(w, 0, maxcol-3);
            WriteChar(w, '[');
            WriteChar(w, CHR(15));
            WriteChar(w, ']');
...
BEGIN
    GetScreenSize (ScreenRows, ScreenColumns);
    FOR p := 0 TO MAX(DisplayPage) DO
    ...
    IF MouseAvailable() THEN
        ...
        SelectPage (0);
    END (*IF*);
    AddActiveRegion (DefaultUIW, 0, ScreenRows-1, 0, ScreenColumns-1,
